...
public sealed class MapDefinition
{
    ...
    /// </summary>
    public static async Task<MapLoadResult> LoadAsync(string fileName, Stream contentStream, CancellationToken cancellationToken = default)
    {
    ...
    /// <summary>
    /// Parses raw .rb3 / .rbp text content into a map definition.
    /// </summary>
    public static MapLoadResult Parse(string rawContent)
    {
        if (string.IsNullOrWhiteSpace(rawContent))
        {
            return new MapLoadResult
            {
                Succeeded = false,
                Errors = new[] { "Map content is empty." }
            };
        }
